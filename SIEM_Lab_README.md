...
An end-to-end SIEM built from scratch to practice the core analyst loop: collect logs, establish a normal baseline, generate attacks, write detections, and catch a brute-force attempt. I built and debugged every layer by hand.
...
For the cron sudo to run without a password prompt, I added a `NOPASSWD` rule, but scoped it to only `/bin/true` and nothing else. Least privilege even in a lab; I didn't want a blanket password-less sudo sitting around.
...
Ran the attack script (6 failures from one IP, over the threshold) and the rule fired, with the alert landing in the queue. Working brute-force detection, end to end.

## Where It Stands
Complete detection loop, end to end: log-source VM to Filebeat to Elasticsearch to Kibana. A benign baseline, an on-demand attack generator, and a threshold rule that catches a brute-force burst. Real auth telemetry is flowing, and I can tell good logins from bad in the data. Built and debugged every layer myself.
